...
void client_loop(int socket_desc, char *current_user) {
  ...
  char choice[32];
  strcpy(choice, "notify\0");   // Start with notify procedure
  ...
  while (1) {   // Starting loop
    ...
    /////////// CREATE TOPIC ///////////
      ...
      // sending all info about the topic
      // Title
    ...
    /////////// ADD THREAD ///////////
      ...
      ret=recv(socket_desc, buf, buf_len, 0);
      ERROR_HELPER(ret, "Recv Error");
      // Sending all info about the Thread to create
      if (strncmp(buf, "content", 7)) {
    ...
    if (!strncmp(buf, "help\0", 4)) {
      ...
      // Print logo and  server's response
      print_logo();
      printf("%s\n", buf);
    }
...
int main(int argc, char *argv[]) {
  ...
  // Receive server's response to A or R
  ret = recv(socket_desc, buf, buf_len, 0);
  ERROR_HELPER(ret, "Recv Error");
